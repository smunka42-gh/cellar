...
| **Fiscal-year-end change** | A company moves its year-end; YoY comparability breaks around the shift, and period alignment wobbles for a year or two | M4, M5, M6, M7 | **Partially guarded → open (custom handling planned).** YoY matches on ~365-day spacing (±45d); a shifted year with no clean comparable reads *no timely read* today. Fiscal-calendar-changers are a **flagged category slated for custom M1–M7 measurement handling** — like the financials/utilities exceptions, not a one-size rule. Rigor-phase build. |
